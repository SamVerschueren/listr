...
class Task {
	...
	constructor(listr, task, options) {
		...
		if (typeof task.skip !== 'undefined' && typeof task.skip !== 'function') {
	...
	run() {
		return Promise.resolve()
			...
			.then(skip => {
				if (skip) {
					const reason = typeof skip === 'string' ? skip : `Skipping ${this.title}.`;
					this.state = state.SKIPPED;
					this._lastLine = stripAnsi(reason.trim().split('\n').pop().trim());
					return;
				}
