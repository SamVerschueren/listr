...
class Task extends Subject {
	constructor(listr, task, options) {
		...
		ow(task, ow.object.label('task'));
		ow(task.title, ow.string.label('task.title'));
		ow(task.task, ow.function.label('task.task'));
		if (task.skip) {
			ow(task.skip, ow.function.label('task.skip'));
		}
		if (task.enabled) {
			ow(task.enabled, ow.function.label('task.enabled'));
		}
